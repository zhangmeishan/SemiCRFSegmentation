...
#ifndef SRC_NNHypridCRF_H_
#define SRC_NNHypridCRF_H_
...
public:
	inline void initial(LookupTable& words, vector<LookupTable>& types, UniParams& tanh1_project, LSTM1Params& left_lstm_project,
		...
		UniParams& olayerbmes_linear, int wordcontext, dtype dropout){
...
public:
	...
	inline void init(int wordcontext, int charcontext, int hiddensize1, int rnnhiddensize, int hiddensize2, int seghiddensize, int seglabelSize, int labelSize) {
		...
		_olayer_linear.initial(_seglabelSize, _seghiddensize, false, 600);
		_olayerbmes_linear.initial(_labelSize, _hiddensize1, false, 700);

		assert(_loss.labelSize == _seglabelSize);

		_bmesloss.initial(_labelSize, 800);
		...
		_pcg->initial(_words, _types, _tanh1_project, _left_lstm_project, _right_lstm_project, _tanh2_project, _seglayer_project, _olayer_linear, _olayerbmes_linear, _wordcontext, _dropOut);
