...
#ifndef SRC_NNSEmbHypridCRF_H_
#define SRC_NNSEmbHypridCRF_H_
...
public:
	inline void initial(LookupTable& words, vector<LookupTable>& types, UniParams& tanh1_project, LSTM1Params& left_lstm_project,
		...
		BiParams& segtanh_project, UniParams& olayer_linear, UniParams& olayerbmes_linear, int wordcontext, dtype dropout){
		for (int idx = 0; idx < word_inputs.size(); idx++) {
			...
			output_bmes[idx].setParam(&olayerbmes_linear);
		}	
...
public:
	...
	inline void init(int wordcontext, int charcontext, int hiddensize1, int rnnhiddensize, int hiddensize2, int seghiddensize, int seglabelSize, int labelSize) {
		...
		_olayer_linear.initial(_seglabelSize, _seghiddensize, false, 700);
		_olayerbmes_linear.initial(_labelSize, _hiddensize1, false, 800);

		assert(_loss.labelSize == _seglabelSize);

		_bmesloss.initial(_labelSize, 900);
		...
		_olayerbmes_linear.exportAdaParams(_ada);
		_bmesloss.exportAdaParams(_ada);


		_pcg = new ComputionGraph();
		_pcg->createNodes(ComputionGraph::max_sentence_length, _loss.maxLen, _types.size());
		_pcg->initial(_words, _types, _tanh1_project, _left_lstm_project, _right_lstm_project, _tanh2_project, _seglayer_project, _segs, _segtanh_project, _olayer_linear, _olayerbmes_linear, _wordcontext, _dropOut);
	...
	inline dtype train(const vector<Example>& examples, int iter) {
		...
		for (int count = 0; count < example_num; count++) {
			...
			cost += _bmesloss.loss(getPNodes(_pcg->output_bmes, seq_size), example.m_labels, _eval, example_num);
	...
	inline void predict(const vector<Feature>& features, NRMat<int>& results, vector<int>& result2s) {
		...
		_bmesloss.predict(getPNodes(_pcg->output_bmes, seq_size), result2s);
	}
	...
	inline dtype cost(const Example& example){
		...
		cost += _bmesloss.cost(getPNodes(_pcg->output_bmes, seq_size), example.m_labels, 1);
