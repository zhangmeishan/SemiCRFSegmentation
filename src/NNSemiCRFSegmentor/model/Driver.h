...
public:
	...
	inline void initial(){
		...
		_pcg -> createNodes(ComputionGraph::max_sentence_length, _hyperParams.maxsegLen, _modelParams._types.size());
	...
	inline dtype train(const vector<Example>& examples, int iter) {
		...
		for (int count = 0; count < example_num; count++) {
			...
			//}
			cost += _modelParams._loss.loss(_pcg->poutput, example.m_seglabels, _eval, example_num);
	...
	inline void predict(const vector<Feature>& features, NRMat<int>& results) {
		...
		//}
		_modelParams._loss.predict(_pcg->poutput, results);
	}
	...
	inline dtype cost(const Example& example){
		...
		//}
		cost += _modelParams._loss.cost(_pcg->poutput, example.m_seglabels, 1);
