...
#include "NNSEmbSemiCRFSegmentor.h"
...
void Segmentor::convert2Example(const Instance* pInstance, Example& exam) {
	...
	for (int idx = 0; idx < segs.size(); idx++){
		...
		if (disIndex < m_options.maxsegLen && orcaleId >= 0) { 
			...
			if (m_driver._hyper_params.maxLabelLength[orcaleId] < disIndex + 1) m_driver._hyper_params.maxLabelLength[orcaleId] = disIndex + 1;
		}
	}
	...
	for (int i = 0; i < curInstSize; ++i) {
		if (labels[i].length() == 1){
			...
			if (m_driver._hyper_params.maxLabelLength[orcaleId] < 1) m_driver._hyper_params.maxLabelLength[orcaleId] = 1;
		}
	}
}
